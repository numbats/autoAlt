...
extract_ggplot_code <- function(file_path) {
  ...
  for (i in seq_along(all_code_chunks)) {
    ...
    if (
      stringr::str_detect(chunk$code, "ggplot|geom") &
        stringr::str_detect(chunk$code, "aes")
    ) {
      # If so, find location of the code chunk in the qmd file and
      original_location <- find_chunk_location(
        content,
        chunk$chunk_label,
        chunk$code
      )
      # Grab the paragraph that references the chunk label (@fig-)
      reference_paragraph <- find_reference_text(
        content,
        chunk$chunk_label,
...
parse_code <- function(purled_content) {
  ...
  for (i in seq_along(chunks)) {
    chunks[[i]]$code <- stringr::str_trim(paste0(
      chunks[[i]]$code,
      collapse = "\n"
    ))
  }
    ...
    if (length(all_syntax) > 0) {
      ...
      if (nchar(first_line) > 0) {
        for (i in seq_along(content)) {
          if (
            stringr::str_detect(
              stringr::str_trim(content[i]),
              stringr::fixed(first_line)
            )
          ) {
            #fixed() to compare literal bytes

            # chunk_start is where "^{r" is
            chunk_start <- i
            while (
              chunk_start > 1 &&
                !stringr::str_detect(content[chunk_start - 1], "^```\\{r")
            ) {
            ...
            while (
              chunk_end > 1 &&
                !stringr::str_detect(content[chunk_end - 1], "^```$")
            ) {
